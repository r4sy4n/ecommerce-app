        ...
        if( dbResponse.length > 0 ){
            ...
            bcrypt.hash( request.body.password, 10 ).then((hash, err) => {
                ...
                newUser.save().then( dbResponse => {
                    response.cookie('token', token, { httpOnly: true, sameSite: 'strict', expiresIn: expirationDate, secure: true, domain: 'ecommerce-app-l8ka.onrender.com', path:'/' }).status( 201 ).send({ message: 'Registration Successful', username: dbResponse.username, email: dbResponse.email, isAdmin: dbResponse.isAdmin, success: true });
                });
            });
        };
    })
    ...
    User.findOne({ email: request.body.email }).select('+password').then( dbResponse => {
        ...
        bcrypt.compare( request.body.password, dbResponse.password ).then( isValid => {
            if( !isValid ){
                ...
                const expirationTime = 24 * 60 * 60 * 1000; // 1 day in milliseconds
                const expirationDate = new Date(Date.now() + COOKIE_EXPIRE_TIME * expirationTime);
                response.cookie('token', token, { httpOnly: true, sameSite: 'strict', expiresIn: expirationDate, secure: true, domain: 'ecommerce-app-l8ka.onrender.com', path:'/' }).status( 200 ).send({
